...
const actorCheck = async (arr: Array<any>, movieid: Number, userRating: Number) => {
  try {
    const result = [];
    for (let i = 0; i < arr.length; i++) {
...
const getMovieWithCredits = async (id: Number): Promise<Array<MovieExtended> | undefined> => {
  try {
...
const actorSort = async (userEmail: String) => {
  try {
    const filter: UserMovieList = await movielist.findOne({
      email: userEmail
    });
...
const onLoadDirectorNoDB = async (): Promise<Array<NewDirectorList> | undefined> => {
  try {
...
const onLoadArrayGenreWithDB = async (array: Array<GenreRating>, user: UserMovieList) => {
  try {
    const page = 1;
    const max = array;
...
const onLoadArrayDirectorWithDB = async (array: Array<GenreRating>, user: UserMovieList) => {
  try {
    let directorArray = [];
    const finalResponse = [];
    const max = array;
    if (max.length >= 2) {
      ...
      finalResponse.push(shuffledMax[0].name);
      let directorHighest = shuffled[0].id
      if (maxDirector === directorHighest) {
        directorHighest = shuffled[1].id;
        finalResponse.push(shuffled[1].name);
      } else {
        finalResponse.push(shuffled[0].name);
      }
    ...
    for (let i = 0; i < directorArray.length; i++) {
      ...
      checkIfInDB(user.movielist, filteredArray);
...
const onLoadArrayActorWithDB = async (array, user) => {
  try {
    let actorArray = [];
    const finalResponse = [];
    const max = array;
 ...
 */
const checkIfInDB = (userDB: Array<MovieExtended>, array: Array<Movie>) => {
...
const onLoad = async (req: Request, res: Response) => {
  try {
    ...
    const genre: Array<GenreRating> | undefined = await genreSort(userEmail) || undefined;
    ...
    const director: Array<DirectorRating> | undefined = await directorSort(userEmail);
    let directorIDArr;
